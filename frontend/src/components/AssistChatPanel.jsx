...
function AssistChatPanel({entities = [], currentEntity, entityType = "person"}) {
    ...
    const contextOptions = [
        ...(hasCurrentEntity
            ? [{
                ...
                label: `${currentEntity.title.trim()} (this entity)`,
                title: currentEntity.title.trim(),
    ...
    const handleSend = () => {
        ...
        chatWithAssistant(entityType, nextMessages, contextEntities)
            .then(result => {
                if (result.error) {
    ...
    return (
        <div className="d-flex flex-column h-100">
            ...
            <div className="flex-grow-1 overflow-auto mb-3 border rounded p-2" style={{minHeight: 200, maxHeight: 320}}>
                {messages.length === 0 && (
                    <p className="text-muted small mb-0">Ask for ideas, lore, or phrasing — copy what's useful into your
                        description.</p>
                )}
                {messages.map((m, i) => (
                    <div key={i}
